...
against a STAR rubric, and writes a coaching report with concrete fixes and drills for the
competencies where you were weak.

The interview adapts per answer. A vague or unquantified story gets a follow-up probe, a
...
Under construction. The engine is being built phase by phase, and nothing is installable yet.
When it ships, the install will be `uvx sotellme` from PyPI.
...
reports stay on your machine; the only data that leaves it is the API traffic to the provider
you picked. Model selection has two internal slots, a fast one for interviewing and a smart
...
Secrets are read only from the environment and never enter prompts; the test suite asserts
this. Langfuse tracing is a dev-time option that activates only when its env vars are set.
